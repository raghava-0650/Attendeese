      ...
      <div className="min-h-screen p-6 bg-gray-100">
        <div className="max-w-3xl mx-auto bg-white shadow-xl rounded-2xl p-6">
          ...
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between">
            <input type="text" placeholder="Search subjects..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="p-2 border rounded-lg w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4 sm:mb-0" />
            <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="bg-white rounded-2xl shadow-md p-4 border-l-4 border-blue-500 mb-6">
            ...
            <p className="text-gray-600">Total Hours Attended: <span className="font-medium">{totalAttendedHours}</span></p>
            <p className="text-gray-600">Total Hours Conducted: <span className="font-medium">{totalClassesHours}</span></p>
            <p className="text-gray-600">Total Attendance Percentage: <span className="font-medium">{totalPercentage}%</span></p>
          </div>
              ...
              sortedSubjects.map((subj, index) => {
                ...
                return (
                  <div key={index} className={`bg-white rounded-2xl shadow-md p-4 border-l-4 ${borderColor}`}>
                    ...
                    <p className="text-gray-600">Classes Attended: <span className="font-medium">{subj.attended}</span></p>
                    <p className="text-gray-600">Classes Absent: <span className="font-medium">{subj.absent}</span></p>
                    <p className="text-gray-600">Total Classes: <span className="font-medium">{subj.total}</span></p>
                    <p className="text-gray-600">Hour Duration per Class: <span className="font-medium">{subj.hourDuration}</span></p>
                    <p className="text-gray-600">Attendance Percentage: <span className="font-medium">{attendancePercentage}%</span></p>
                    ...
                    <div className="flex justify-end space-x-4 mt-2">
                      <button onClick={() => handleEditSubject(index)} className="text-blue-600 hover:text-blue-800 transition">Edit</button>
                      <button onClick={() => showDeleteConfirmation(index)} className="text-red-600 hover:text-red-800 transition">Delete</button>
                    </div>
                    {editingIndex === index && (
                      <div className="mt-4">
                        <input type="number" placeholder="Edit Classes Attended" value={editingAttended} onChange={(e) => setEditingAttended(e.target.value)} className="p-2 border rounded-lg mb-3 w-full focus:outline-none focus:ring-2 focus:ring-green-500" />
                        <input type="number" placeholder="Edit Classes Absent" value={editingAbsent} onChange={(e) => setEditingAbsent(e.target.value)} className="p-2 border rounded-lg mb-3 w-full focus:outline-none focus:ring-2 focus:ring-red-500" />
                        <textarea placeholder="Edit Note (optional)" value={editingNote} onChange={(e) => setEditingNote(e.target.value)} className="p-2 border rounded-lg mb-3 w-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        <button onClick={handleSaveEdit} className="w-full bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition">Save Changes</button>
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-gray-700 bg-opacity-50 transition-opacity duration-300">
              <div className="bg-white p-6 rounded-xl shadow-xl w-96">
                ...
                <div className="flex justify-end mt-4">
                  <button onClick={hideDeleteConfirmation} className="mr-4 text-gray-600 hover:text-gray-800 transition">Cancel</button>
                  <button onClick={handleDeleteSubject} className="bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-700 transition">Delete</button>
                </div>
              </div>
            </div>
          )}
